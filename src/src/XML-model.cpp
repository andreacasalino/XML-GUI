...
void printTag(gui::json::arrayJSON& nodes, const std::size_t& counter, const std::string& name, const bool& isRoot = false){
...
void printAttribute(gui::json::arrayJSON& nodes, const std::size_t& counter, const std::string& name, const std::string& value){
  gui::json::structJSON s;
...
void printEdge(gui::json::arrayJSON& edges, const std::size_t& from, const std::size_t& to, const bool& leads2Tag){
  gui::json::structJSON s;
...
void XML_model::updateJsonTag(gui::json::arrayJSON& nodes, gui::json::arrayJSON& edges, std::size_t& counter, const xmlPrs::Tag& tag, const xmlNodePosition& parentPosition, const std::size_t& parentId) {
...
std::string XML_model::Rename(const gui::RequestOptions& opt){
  FIND_OPT_O(this->GetJSON())
  if(itOpt->second.size() < 2) return this->GetJSON();
  std::size_t id = std::atoi(itOpt->second[0].c_str());
  auto it = this->nodesInfo.find(id);
  if(it != this->nodesInfo.end()){
    xmlPrs::Tag& tag = this->data.getRoot().getNested(it->second.pathFromRoot);
...
std::string XML_model::NestTag(const gui::RequestOptions& opt){
  ...
  if(it != this->nodesInfo.end() && (nullptr == it->second.attributeName)){
    xmlPrs::Tag& tag = this->data.getRoot().getNested(it->second.pathFromRoot);
    std::cout << "nest tag " << itOpt->second[1] << " to tag " << tag.getName() << std::endl;
...
std::string XML_model::NestAttribute(const gui::RequestOptions& opt){
  ...
  if(it != this->nodesInfo.end() && (nullptr == it->second.attributeName)){
    xmlPrs::Tag& tag = this->data.getRoot().getNested(it->second.pathFromRoot);
    std::cout << "nest attribute " << itOpt->second[1] << " to tag " << tag.getName() << std::endl;
